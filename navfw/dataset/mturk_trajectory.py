...
from navfw.scr.space import Point3D, Pose5D
from navfw.configs.dronenav.defaultpaths import MTURK_TRAJECTORY_DIR
from navfw.scr.trajectory import straight_line_trajectory
...
class MTurkTrajectory:
    ...
    def interpolated_trajectory(self):
        interpolated_trajectory = [self.start_pose.xyz]
        for src, dst in zip(self.extended_trajectory[:-1], self.extended_trajectory[1:]):
            for pos in straight_line_trajectory(src, dst):
                if interpolated_trajectory[-1].dist_to(pos) > 5.:
                    interpolated_trajectory.append(pos)

            if interpolated_trajectory[-1] != dst:
                interpolated_trajectory.append(dst)
